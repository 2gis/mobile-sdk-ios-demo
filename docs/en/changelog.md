...
- *Ломающее изменение*. Изменены названия пакетов - `PlatformSDK` и `PlatformMapSDK` переименованы в `DGis` независимо от типа сборки NSDK.
- Уменьшен размер Map-версии NSDK - убрали работу с offline справочником и источником данных карты.
