...
- Реализован контрол `NextObstacleControlView` для отображения приближения к различным препятствиям (авто, пешеход), а также к камерам и прохождениям их области видимости (для авто). Получить объект контрола можно с помощью `INavigationViewControlsFactory.makeNextObstacleControlControl`.
