...
- *Ломающее изменение*. Изменены названия пакетов - `PlatformSDK` и `PlatformMapSDK` переименованы в `DGis` и `DGisMap` соответственно.
- Уменьшен размер `DGisMap` пакета - убрали работу с offline справочником и источником данных карты.

  Для `DGisMap` в `ISearchManagerFactory` остался только метод `makeOnlineManager` для создания online справочника, а `makeOfflineManager` и `makeSmartManager` теперь доступны только в `DGis` пакете.

  Для `DGisMap` в `ISourceFactory` удален метод `createOfflineDGISSource` для создания offline источника данных карты.
