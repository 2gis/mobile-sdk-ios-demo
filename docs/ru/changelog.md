...
- При создании объектов карты появилась возможность задавать порядок их отрисовки через `MarkerOptions.zIndex`, `PolygonOptions.zIndex` и т.д. Чем выше значение индекса, тем позднее объект появится на карте.
- Упрощение создания/преобразования простых типов данных, таких как `Arcdegree`, `Color`, `Elevation`, `GeoPoint` и т.д.

  Например, помимо варианта создания объекта типа GeoPoint
  val geoPoint = GeoPoint(Arcdegree(1.0), Arcdegree(2.0))
  также доступен вариант
  val geoPoint = GeoPoint(1.0, 2.0).

- Добавлена возможность задавать список локалей в обход системным настройкам с помощью `LocaleManager.setOverridedLocales`. Для возврата системных локалей необходимо использовать `LocaleManager.resetLocalesToSystemLocales`.
- Добавлен метод `toMapGeometry(trafficRoute, currentRoutePoint)` для получения геометрии оставшейся части маршрута.
- Исправлено отображение карты при нулевом z-уровне.
- Добавлена возможность отключения анимации при появлении или исчезании объектов на карте через `MarkerOptions.animatedAppearance` и `Marker.setAnimatedAppearance`.
- *Ломающее изменение*. Удален устаревший класс `MarkerBuilder` для создания объектов карты. Вместо него нужно использовать класс `MapObjectManager` для добавления объектов, а также `Marker`, `Polygon` и т.д. для их создания.
- *Ломающее изменение*. Поддержана установка корректного DPI при создании карты. Теперь при установке DPI через `MapBuilder.setDevicePpi` при создании карты необходимо задать множитель, который вычисляется как отношение devicePPI к базовому DPI устройства. Значение множителя соответствует nativeScale целевого экрана.
