...
- В `MapObjectManager` появился метод `clusteringObjects(сameraPosition:)` для получения объектов кластеризации для заданной позиции камеры. Метод возвращает как кластеры `ISimpleClusterObject`, так и маркеры `IMarker`.
