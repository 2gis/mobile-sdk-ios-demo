...
- Помимо обновленного дизайна контролов в новом навигаторе:
  ...
  - Добавлен новый контрол `IThermometerControlView` для отображения градусника маршрута в навигаторе.
...
Для всех типов Mobile SDK:
  - Нужно обратится в техническую поддержку 2ГИС для получения нового ключа для использования MobileSDK. Обязательно нужно указать `appId` приложения, для которого будет создан ключ. Полученный файл ключа `dgissdk.key` нужно добавить либо в корень приложения, либо при инициализации MobileSDK передать объект структуры `ApiKeyOptions` с указанием пути до файла.
  - Обновить стили.
