...
Map-версия и Full-версия:
  - Нужно обратится в техническую поддержку 2ГИС для получения нового ключа для использования MobileSDK. Полученный файл ключа `dgissdk.key` нужно добавить либо в корень приложения, либо при инициализации MobileSDK передать объект структуры `ApiKeyOptions` с указанием пути до файла.
  - Обновить стили
