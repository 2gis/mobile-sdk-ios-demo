...
public final class CustomZoomControl: UIControl {
	override public static var requiresConstraintBasedLayout: Bool { true }
...
private final class ZoomButton: UIButton {
	...
	init(model: ZoomControlModel, direction: ZoomControlButton) {
		...
		self.connection = self.model.isEnabled(button: self.direction).sinkOnMainThread { [weak self] isEnabled in
