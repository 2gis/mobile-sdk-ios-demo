...
struct SearchView: View {
	...
	var body: some View {
		...
		.background(Color("colors/secondary_background").ignoresSafeArea())
	}
		...
		Text(rubric.name)
			...
			.background(
				RoundedRectangle(cornerRadius: 8)
					.fill(self.store.state.rubricIds.firstIndex(of: rubric.value) != nil ? .gray : Color("colors/background"))
			)
			.onTapGesture {
	...
	private var voiceAssistantOverlay: VoiceAssistantView {
		self.directoryViewsFactory.makeVoiceAssistantView(speechService: nil)
			...
			.recognitionCompleteCallback(callback: { result in
				if case let .finished(text) = result {
					if let text {
